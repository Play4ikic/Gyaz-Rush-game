...
window.startOnlineSearch = async function() {
    ...
    if (!snapshot.exists()) {
        ...
        searchTimeout = setTimeout(async () => {
            const currentQueue = await get(queueRef);
            if (currentQueue.exists() && currentQueue.val().hostId === userData.uid) {
                await remove(queueRef);
                await remove(ref(db, `matches/${matchId}`));
                off(ref(db, `matches/${matchId}/status`)); // Перестаем слушать
                statusText.innerText = "НЕТ ИГРОКОВ В СЕТИ";
                alert("Соперник не найден. Попробуйте позже.");
            }
        }, 15000);
        ...
        myRole = "guest";
        opponentName = data.hostName; // Запоминаем имя хоста
...
function listenForOpponent() {
    const statusRef = ref(db, `matches/${matchId}`);
    onValue(statusRef, (snap) => {
        const data = snap.val();
        if (data && data.status === "playing") {
            clearTimeout(searchTimeout);
            opponentName = data.guestName; // Запоминаем имя зашедшего гостя
            initGameUI();
        }
    });
}
...
function initGameUI() {
    ...
    // Обновляем заголовок счета и имя противника над картой
    const scoreText = document.querySelector('.score');
    scoreText.innerHTML = `${userData.nickname.toUpperCase()} <span id="p-score">0</span> : <span id="b-score">0</span> ${opponentName.toUpperCase()}`;
    
    document.querySelectorAll('.card-zone h3')[1].innerText = opponentName.toUpperCase();
    startRound();
}
...
function processBattle() {
    ...
    setTimeout(() => {
        currentRound++;
        off(ref(db, myRole === "host" ? `matches/${matchId}/round${currentRound}/guestCard` : `matches/${matchId}/round${currentRound}/hostCard`));
        startRound();
    }, 3000);
}
...
async function endMatch() {
    ...
    if (myScore > oppScore) {
        resultText = `ПОБЕДА НАД ${opponentName}! +5000 CY`;
        reward = 5000;
    } else if (myScore < oppScore) {
        resultText = `ПРОИГРЫШ ОТ ${opponentName}.`;
        reward = 0;
    } else {
        resultText = "НИЧЬЯ! +500 CY";
        reward = 500;
    }
